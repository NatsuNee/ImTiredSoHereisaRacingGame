                ...
                if (Event.current.control) {
                    ...
                    for (var i = 0; i < points.Count; i++) {
                        ...
                        if (i == points.Count - 1) {
                            ...
                            var fmh_352_109_638285943482683363 = Quaternion.identity; points[i] = new SplineStamp.SplinePoint(Handles.FreeMoveHandle(points[i].point, HandleUtility.GetHandleSize(points[i].point) * handleScale * 2f, Vector3.zero, Handles.CubeHandleCap), points[i].scale); ;

                        } else {

                            var fmh_356_109_638285943482709333 = Quaternion.identity; points[i] = new SplineStamp.SplinePoint(Handles.FreeMoveHandle(points[i].point, HandleUtility.GetHandleSize(points[i].point) * handleScale * 2f, Vector3.zero, Handles.SphereHandleCap), points[i].scale);
